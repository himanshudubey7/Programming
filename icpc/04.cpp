...
void solve(){
  ll t;
    ...
    while(t--){
        ...
        // if(cities[c]>=cities[b]){
        //     cout<<p* abs(cities[b]-cities[a])<<endl;
        //     continue;
        // }
        ll direct = p * abs(cities[b]-cities[a]);
        // cout<<"direct = "<<direct<<endl;
        ll indirect = p* abs(cities[c]-cities[a]);
        ll rem = y - indirect;
        if(rem<0){
            cout<<direct<<endl;
            continue;
        }
        indirect+= (rem+ (q*abs(cities[c]-cities[d])) + (p* abs(cities[d]-cities[b])));
        ll ans = min(direct , indirect);
